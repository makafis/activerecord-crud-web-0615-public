...
def can_get_the_first_item_in_the_database
  Movie.select(:title).limit(1)[0].title
end

def can_get_the_last_item_in_the_database
  # require 'pry'
  Movie.select(:title).last.title
...
def can_find_by_different_attributes
  ...
  # release_date == 2000, 
  # director == "Me"
  Movie.where(title: 'Title').where(release_date: 2000).where(director: 'Me')[0]
end
